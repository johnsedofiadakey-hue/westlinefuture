...
export default function PaystackPayModal({ invoice, brand, onClose, onSuccess }) {
  ...
  if (status === 'error') {
    return (
      <div style={{ position: 'fixed', inset: 0, background: 'rgba(17,24,39,.9)', backdropFilter: 'blur(12px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
        <div className="p-card fade-in" style={{ width: '100%', maxWidth: 440, padding: 48, textAlign: 'center', background: '#fff', borderRadius: 32 }}>
          ...
          <h2 className="lxfh" style={{ fontSize: 24, color: '#111827', marginBottom: 12 }}>Verification Failed</h2>
          <p className="lxf" style={{ color: '#4B5563', marginBottom: 32, lineHeight: 1.6, fontSize: 14 }}>{error}</p>
  ...
  if (status === 'verifying') {
    return (
      <div style={{ position: 'fixed', inset: 0, background: 'rgba(17,24,39,.9)', backdropFilter: 'blur(12px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
        <div className="p-card fade-in" style={{ width: '100%', maxWidth: 440, padding: 48, textAlign: 'center', background: '#fff', borderRadius: 32 }}>
          <Spinner />
          <h2 className="lxfh" style={{ fontSize: 22, color: '#111827', marginTop: 24, marginBottom: 12 }}>Verifying with Paystack...</h2>
          <p className="lxf" style={{ color: '#4B5563', fontSize: 14 }}>Please wait while we confirm your payment on our servers.</p>
          {verifyRef && <p style={{ fontSize: 11, color: '#6B7280', marginTop: 12 }}>Ref: {verifyRef}</p>}
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={{ position: 'fixed', inset: 0, background: 'rgba(17,24,39,.9)', backdropFilter: 'blur(12px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
        <div className="p-card fade-in" style={{ width: '100%', maxWidth: 440, padding: 48, textAlign: 'center', background: '#fff', borderRadius: 32 }}>
          ...
          <h2 className="lxfh" style={{ fontSize: 28, color: '#111827', marginBottom: 12 }}>Payment Secured</h2>
          <p className="lxf" style={{ color: '#4B5563', marginBottom: 32, lineHeight: 1.6 }}>Transaction successfully authorized. Your invoice <b>{invoice.id}</b> is now cleared in the Westline Future ledger.</p>
          <button onClick={onClose} className="p-btn-dark lxf" style={{ width: '100%', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(17,24,39,.7)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div className="p-card slide-up" style={{ width: '100%', maxWidth: 480, background: '#fff', borderRadius: 28, overflow: 'hidden', boxShadow: '0 32px 64px -16px rgba(0,0,0,.25)' }}>
        ...
        <div style={{ padding: '32px 32px 24px', position: 'relative' }}>
           <button onClick={onClose} style={{ position: 'absolute', top: 24, right: 24, background: '#F9FAFB', border: 'none', width: 32, height: 32, borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#6B7280' }}><X size={18} /></button>
           <div className="eyebrow" style={{ color: ac, marginBottom: 8 }}>Secure Checkout</div>
           <h2 className="lxfh" style={{ fontSize: 24, color: '#111827' }}>Paystack Gateway</h2>
        </div>
        ...
        <div style={{ padding: '0 32px 32px' }}>
           ...
           <div style={{ padding: 20, background: '#F9FAFB', borderRadius: 16, marginBottom: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                 <div className="lxf" style={{ fontSize: 13, color: '#4B5563' }}>{invoice.title}</div>
                 <div className="lxf" style={{ fontSize: 13, fontWeight: 700, color: '#111827' }}>{invoice.amount}</div>
              </div>
              <div className="lxf" style={{ fontSize: 11, color: '#6B7280' }}>Invoice ID: {invoice.id}</div>
           </div>
           ...
           <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <button
                ...
                style={{ width: '100%', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, borderRadius: 16, background: status !== 'idle' ? '#E5E7EB' : '#0F766E', color: status !== 'idle' ? '#6B7280' : '#fff', border: 'none', cursor: status !== 'idle' ? 'default' : 'pointer' }}
              >
           ...
           <div style={{ display: 'flex', justifyContent: 'center', gap: 24, marginTop: 24, opacity: 0.5 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 10, color: '#111827', fontWeight: 600 }}>
