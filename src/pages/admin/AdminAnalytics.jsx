    ...
    <div className="fade-in" style={{ padding: '20px 0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 32 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <select
            ...
            style={{ height: 38, borderRadius: 10, border: '1px solid #E8E6F5', padding: '0 12px', fontSize: 12, fontWeight: 700, background: '#fff', color: '#0D0B2E', cursor: 'pointer' }}
          >
            {availableYears.map(y => <option key={y} value={y}>{y}</option>)}
          </select>
          <div style={{ display: 'flex', borderRadius: 10, border: '1px solid #E8E6F5', overflow: 'hidden' }}>
            {['All', 'Q1', 'Q2', 'Q3', 'Q4'].map(q => (
              <button
                key={q}
                onClick={() => setSelectedQuarter(q)}
                style={{ padding: '8px 14px', border: 'none', borderRight: q !== 'Q4' ? '1px solid #E8E6F5' : 'none', background: selectedQuarter === q ? '#0D0B2E' : '#fff', color: selectedQuarter === q ? '#fff' : '#5B5894', fontSize: 11, fontWeight: 800, cursor: 'pointer' }}
              >{q}</button>
            ))}
          </div>
          ...
          <div style={{ width: '100%', height: 280 }}>
            <ResponsiveContainer>
              <AreaChart data={revenueData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <YAxis stroke="#9B99C8" fontSize={11} tickFormatter={v => `GH₵${(v/1000).toFixed(0)}k`} />
      ...
      <div className="p-card" style={{ border: '1px solid #E8E6F5', overflow: 'hidden' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid #E8E6F5', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={handleExportInvoices} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: '#F8F8FD', border: '1px solid #E8E6F5', borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>
            <Download size={13} /> Export CSV
          </button>
        </div>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead style={{ background: '#F8F8FD' }}>
              <tr>
            ...
            <tbody>
              {invoices.slice(0, 10).map(inv => (
                <tr key={inv.id} style={{ borderBottom: '1px solid #F8F8FD' }}>
