...
export default function AdminSystem({ onReset, syncCatalog, brand }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 32, maxWidth: 800 }}>
       <div>
          <h2 className="lxfh" style={{ fontSize: 32, fontWeight: 400, color: `var(--accent-secondary)` }}>System Maintenance</h2>
          <p className="lxf" style={{ color: `var(--text-secondary)`, fontSize: 14 }}>Global environment controls and data synchronization tools.</p>
       </div>

        <div className="p-card" style={{ padding: 32, border: confirm ? `2px solid #EF4444` : '1px solid var(--border)' }}>
          <div style={{ display: 'flex', gap: 24 }}>
             <div style={{ width: 64, height: 64, borderRadius: 20, background: confirm ? '#FEF2F2' : `var(--bg-secondary)`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: confirm ? '#EF4444' : ac }}>
                <Database size={32} />
             </div>
             <div style={{ flex: 1 }}>
       ...
       <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
          <div className="p-card" style={{ padding: 24 }}>
             ...
             <p className="lxf" style={{ fontSize: 12, color: `var(--text-secondary)`, marginBottom: 20 }}>Verify Firestore rules and storage bucket permissions.</p>
          ...
          <div className="p-card" style={{ padding: 24 }}>
             ...
             <p className="lxf" style={{ fontSize: 12, color: `var(--text-secondary)`, marginBottom: 20 }}>Export current database snapshot to JSON format.</p>
             <button className="p-btn-light lxf" style={{ width: '100%', fontSize: 12 }}>Generate Backup</button>
          </div>
       </div>

       <div className="p-card" style={{ padding: 32, background: `var(--accent-secondary)`, color: '#fff' }}>
