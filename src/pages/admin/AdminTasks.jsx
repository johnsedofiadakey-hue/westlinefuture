...
export default function AdminTasks({ projectId, projectTitle, tasks, createTask, deleteTask, updateTask, teamMembers, brand, notify }) {
  ...
  return (
    <div className="p-card" style={{ padding: 24 }}>
      ...
      {showAdd && (
        <div style={{ background: `var(--bg-secondary)`, padding: 16, borderRadius: 8, marginBottom: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {projectTasks.length === 0 && <div className="lxf" style={{ color: `var(--text-secondary)`, fontSize: 12, textAlign: 'center', padding: '10px 0' }}>No tasks found</div>}
        {projectTasks.map(t => (
          <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px', background: `var(--bg-secondary)`, borderRadius: 8 }}>
             ...
             <div style={{ flex: 1 }}>
                <div className="lxf" style={{ fontSize: 13, fontWeight: 500, textDecoration: t.status === 'completed' ? 'line-through' : 'none', color: t.status === 'completed' ? `var(--text-secondary)` : `var(--accent-secondary)` }}>{t.title}</div>
                <div className="lxf" style={{ fontSize: 10, color: `var(--text-secondary)` }}>{teamMembers.find(m=>m.id === t.assignedTo)?.name || 'Unassigned'} • Stage {t.stage}</div>
             </div>
