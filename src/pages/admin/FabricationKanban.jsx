...
const STAGES = [
  { id: 'queue', label: 'In Queue', color: '#9B99C8', desc: 'Awaiting scheduling' },
  ...
  { id: 'ready', label: 'Ready', color: '#0D0B2E', desc: 'Dispatch stage' }
];
...
export default function FabricationKanban({ jobs = [], clients = [], dbClients = [], updateJob, createJob, sendWhatsAppUpdate, brand, ...props }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      {!props.isSubView && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'center', flexDirection: isMobile ? 'column' : 'row', gap: 16 }}>
          <div>
             ...
             <p className="lxf" style={{ color: '#9B99C8', fontSize: 13 }}>Real-time fabrication command center.</p>
          </div>
                 ...
                 {jobs.filter(j => j.stage === s.id).map(j => (
                   ...
                   >
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
                         <div>
                            <div style={{ fontSize: 9, color: '#9B99C8', fontWeight: 800, textTransform: 'uppercase', marginBottom: 2 }}>{j.id}</div>
      ...
      {showNewJob && (
        <div className="overlay-modal" style={{ position: 'fixed', inset: 0, background: 'rgba(13,11,46,0.7)', backdropFilter: 'blur(8px)', zIndex: 9000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div className="p-card" style={{ width: '100%', maxWidth: 480, padding: 32, borderRadius: 24 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: '#9B99C8', display: 'block', marginBottom: 6 }}>PROJECT TITLE</label>
              ...
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: '#9B99C8', display: 'block', marginBottom: 6 }}>GLASS ITEM DESCRIPTION</label>
              ...
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: '#9B99C8', display: 'block', marginBottom: 6 }}>PRIORITY</label>
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
                ...
                <button
                  ...
                  style={{ flex: 1, height: 48, borderRadius: 12, background: ac, color: '#0D0B2E', border: 'none', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}
                >
                  Add to Queue
                </button>
              </div>
